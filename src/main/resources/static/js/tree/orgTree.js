    ...
    oTree.prototype = {
        dialog: function(){
            ...
            $.ajax({
                ...
                dataType: "json",
                async: false,
                url: "/sys/permissions/findAllPers",//url
                success: function (rt) {
                    if (rt.code =="200" ) {
                        delete rt.code;
                        delete  rt.message;
            ...
            _this.$element.on('click', '.js_show_party_selector', function(){
                ...
                layer.confirm(contentsHTML, {
                    ...
                    $.ajax({
                        ...
                        type: "POST",//方法类型
                        dataType: "json",//预期服务器返回的数据类型
                        url: "/sys/roles/assginPermission",//url
                        data: {
                            "rid":roleid,
                            "ids[]": arr
                        },
                ...
                function fillNode(zNode, dataNode, presentName) {
                    dataNode.forEach(function(aDate){
                        var thisNode = {
                            id: aDate.id,
                            name: aDate.perName,
                            spellName: 'achild_' + spellChange(aDate.perName),
                            parentId: aDate.parent_id,
                            crumbs: presentName +'/'+ aDate.perName,
                            children: []
                        };
                        var noChildNode = {
                            id: aDate.id,
                            name: aDate.perName,
                            spellName: 'bchild_' + spellChange(aDate.perName),
                            parentId: aDate.parent_id,
                            crumbs: presentName +'/'+ aDate.perName,
                        };
